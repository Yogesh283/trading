/**
 * Copy UpDownFX.apk → releases/UpDownFX.apk (VPS deploy: upload this folder or single file via SFTP).
...
const dest = path.join(repoRoot, "releases", "UpDownFX.apk");
...
for (const src of sources) {
  if (fs.existsSync(src)) {
    ...
    console.log("[copy-apk-to-releases] → releases/UpDownFX.apk ←", path.relative(repoRoot, src));
    process.exit(0);
  }
}
...
console.error(
  "[copy-apk-to-releases] No APK found. Do one of:\n" +
    ...
    "  3) Manually copy any UpDownFX.apk to releases/UpDownFX.apk"
);
process.exit(1);
