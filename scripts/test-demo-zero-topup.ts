/**
 * Local check: demo_balance 0 → saveDemoBalanceToDb stays at 0 (daily claim required);
...
async function main() {
  ...
  await dbRun("UPDATE wallets SET demo_balance = 0, demo_hold_zero = 0 WHERE user_id = ?", [uid]);
  const afterZero = await saveDemoBalanceToDb(uid, 0);
  const ok1 = Math.abs(afterZero) < 0.02;
  console.log(`[1] saveDemoBalanceToDb(0) => ${afterZero} (expect 0) ${ok1 ? "OK" : "FAIL"}`);
